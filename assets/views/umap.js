/* IQC · FIELD ATLAS — Map view (canvas UMAP scatter, terminal palette) */
(function () {
  const Atlas = (window.Atlas = window.Atlas || {});
  let canvas, ctx, pts = [], dpr = 1;
  ...
  function toScreen(p) { return { sx: p.x*view.scale+view.x, sy: p.y*view.scale+view.y }; }
  ...
  function draw() {
    ...
    if (hover) {
      ...
      ctx.beginPath(); ctx.arc(sx, sy, 9*dpr, 0, Math.PI*2); ctx.stroke();
      const node = Atlas.data.byId(hover.id);
      if (node) {
        ctx.font = `500 ${11*dpr}px JetBrains Mono`;
  ...
  Atlas.map = Atlas.umap = {
    init() {
      ...
      pts = Atlas.data.get().umap.map((u) => ({ id: u.id, x: u.x, y: u.y }));
      fit();
